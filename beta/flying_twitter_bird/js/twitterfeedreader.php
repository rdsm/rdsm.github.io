...
if(!file_exists($tweetCache)){
	die('Error: cache file does not exist for user "'.$twitterAccount.'"');
}

// gecachter Inhalt laden
...
// Expires-Header senden (=> weniger Datentransfehr)
header('Expires: '.gmdate('D, d M Y H:i:s', time()+$cacheMaxAge).' GMT');

// prüfen, ob Cache noch aktuell
if(filemtime($tweetCache)>=(time()-$cacheMaxAge) && !empty($newestTweet)){
...
// Feed-URL
$feedUrl = 'http://api.twitter.com/1/statuses/user_timeline/'.$twitterAccount.'.rss';
